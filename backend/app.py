...
# Expanded to support up to 500 winning cards - fill dynamically
# Original 100 cards as seed base
WINNING_CARDS_BASE = [
...
# Generate additional 400 cards to reach 500 total
def generate_winning_cards(base_cards, target_count=500):
    """Generate winning cards up to target_count by filling gaps intelligently"""
    winning = set(base_cards)

    # Fill gaps evenly across the range
    if len(winning) < target_count:
        total_range = CARD_MAX - CARD_MIN + 1
        cards_needed = target_count - len(winning)
        step = max(1, total_range // cards_needed)

        for i in range(CARD_MIN, CARD_MAX + 1, step):
            if len(winning) >= target_count:
                break
            card = f"{CARD_PREFIX}{i:04d}"
            if card not in winning:
                winning.add(card)

    return sorted(list(winning))[:target_count]

WINNING_CARDS_100 = WINNING_CARDS_BASE
# Generate 500 winning cards for full seeding
WINNING_CARDS_500 = generate_winning_cards(WINNING_CARDS_BASE, 500)
...
def validate_and_build_prize_map(winning_cards: list[str]) -> dict[str, dict]:
    ...
    # 40%
    count_1 = int(total * 0.40)
    for cn in ordered[idx: idx + count_1]:
        prize_map[cn] = {"prize": PRIZE_TYPES["10% Discount"], "partner_name": PARTNER_TASTY}
    idx += count_1

    # 25%
    count_2 = int(total * 0.25)
    for cn in ordered[idx: idx + count_2]:
        prize_map[cn] = {"prize": PRIZE_TYPES["Free Drink"], "partner_name": PARTNER_LAMBRISCO}
    idx += count_2

    # 25%
    count_3 = int(total * 0.25)
    for cn in ordered[idx: idx + count_3]:
        prize_map[cn] = {
...
def ensure_seed_data():
    ...
    # Always enforce the 500 winning cards prize distribution (idempotent with UPDATE).
    prize_map = validate_and_build_prize_map(WINNING_CARDS_500)
    ...
    # Ensure cards NOT in the 500 list are not winners (optional safety)
    # Only apply if table already had other winners.
    placeholders = ",".join(["?"] * len(WINNING_CARDS_500))
    if placeholders:
        db.execute(
            f"""
            UPDATE coupon_cards
